...
module RecordStubGenerator =
    ...
    let tryCreateFromRecordExpression (expr: RecordExpr) =
    ...
    let private tryFindRecordBindingInParsedInput (pos: pos) (parsedInput: ParsedInput) =
        ...
        and walkSynMemberDefn (memberDefn: SynMemberDefn) =
            getIfPosInRange memberDefn.Range (fun () ->
                ...
                | SynMemberDefn.AutoProperty(_attributes, _isStatic, _id, _type, _memberKind, _memberFlags, _xmlDoc, _access, expr, _r1, _r2) -> walkExpr expr
            ...
            getIfPosInRange binding.RangeOfBindingAndRhs ^ fun () ->
                ...
                | Some(SynBindingReturnInfo(_ty, _range, _attributes)) ->
                    ...
                    | SynExpr.Typed(SynExpr.Record(_inheritOpt, copyOpt, fields, _range0), synType, _range1) ->
                        ...
                        |> Option.orTry ^ fun () ->
                            fields |> List.tryPick walkRecordField
                    | _ -> walkExpr expr
                | None -> walkExpr expr
        and walkExpr expr =
            getIfPosInRange expr.Range ^ fun () ->
                        ...
                        |> List.tryPick ^ fun range ->
                            getIfPosInRange range ^ fun () ->
                                Some { Expr = expr
                                       CopyExprOption = copyOpt
                                       FieldExprList = fields }
                        |> Option.orTry ^ fun () -> fields  |> List.tryPick walkRecordField
                    | _ -> walkExpr synExpr
                ...
                | SynExpr.DotIndexedGet(synExpr, IndexerArgList synExprList, _range, _range2)  -> Option.orElse (walkExpr synExpr) (List.tryPick walkExpr synExprList) 
                ...
                | SynExpr.DotNamedIndexedPropertySet(synExpr1, _longIdent, synExpr2, synExpr3, _range) ->  List.tryPick walkExpr [synExpr1; synExpr2; synExpr3]
                ...
                | SynExpr.LetOrUseBang(_sequencePointInfoForBinding, _, _, _synPat, synExpr1, synExpr2, _range) -> List.tryPick walkExpr [synExpr1; synExpr2]
    ...
    let tryFindStubInsertionParamsAtPos (codeGenService: ICodeGenerationService<'Project, 'Pos, 'Range>) project (pos: 'Pos) document =
        asyncMaybe {
            ...
            if checkThatRecordExprEndsWithRBrace codeGenService project document recordExpression then
                let! insertionPos = tryCreateFromRecordExpression recordExpression

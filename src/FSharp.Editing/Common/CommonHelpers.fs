...
[<RequireQualifiedAccess>]
type internal SymbolRangeLookup =
...
module internal CommonHelpers =
    ...
    let isValidNameForSymbol (lexerSymbolKind: LexerSymbolKind, symbol: FSharpSymbol, name: string) : bool =
        ...
        let isFixableIdentifier (s: string) = 
            not (String.IsNullOrEmpty s) && LexHelp.NormalizeIdentifierBackticks s |> isIdentifier
...
module internal Extensions =
    ...
    type FSharpChecker with
        member this.ParseDocument(document: Document, options: FSharpProjectOptions, sourceText: string) :  Async<ParsedInput option> =
            asyncMaybe {

module FSharp.Editing.Symbols.SymbolHelpers
...
let changeAllSymbolReferences (document: Document, symbolSpan: TextSpan, textChanger: string -> string, projectInfoManager: ProjectInfoManager, checker: FSharpChecker)
    ...
    asyncMaybe {
        ...
        let! symbol = getSymbolAtPosition(document.Id, sourceText, symbolSpan.Start, document.FilePath, defines, SymbolRangeLookup.Greedy)
                ...
                async {
                    ...
                    for KeyValue(documentId, symbolUses) in symbolUsesByDocumentId do
                        ...
                        for symbolUse in symbolUses do
                            let textSpan = fixupSpan(sourceText, fsharpRangeToTextSpan sourceText symbolUse.RangeAlternate)

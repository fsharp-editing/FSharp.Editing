module FSharp.Editing.Symbols.SignatureHelp
...
//let  ProvideMethodsAsyncAux(checker: FSharpChecker, documentationBuilder: IDocumentationBuilder, sourceText: SourceText, caretPosition: int, options: FSharpProjectOptions, triggerIsTypedChar: char option, filePath: string, textVersionHash: int) = async {
...
////                    ,   FSharpToolTipElement methodic
////                    .   StructuredDescription, false
////                    )

//            let parameters = 
//                let parameters = if isStaticArgTip then methodic.StaticParameters else methodic.Parameters
//                [| for p in parameters do 
//                      let doc = List()
//                      // FSROSLYNTODO: compute the proper help text for parameters, c.f. AppendParameter in XmlDocumentation.fs
//                      XmlDocumentation.BuildMethodParamText(documentationBuilder, CollectTaggedText doc, methodic.XmlDoc, p.ParameterName) 
//                      let parts = List()
//                      renderL (taggedTextListR (CollectTaggedText parts)) p.StructuredDisplay |> ignore
...
//            let completionItem = (methodic.HasParamArrayArg, documentation, prefixParts, separatorParts, suffixParts, parameters, mainDescription)
...
//let IsTriggerCharacter(c) = c ='(' || c = '<' || c = ',' 
//let IsRetriggerCharacter(c) = c = ')' || c = '>' || c = '='
////
////
////let GetItemsAsync(document, position, triggerInfo, cancellationToken) =  asyncMaybe {
////    try
...
////        ProvideMethodsAsyncAux(checkerProvider.Checker, documentationBuilder, sourceText, position, options, triggerTypedChar, document.FilePath, textVersion.GetHashCode())
////        let items = 
////                    results 
